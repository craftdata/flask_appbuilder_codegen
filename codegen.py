...
def _render_column(column, show_name):
    ...
    if column.server_default:
        server_default = 'autoincrement=True'# + _flask_prepend + 'FetchedValue()'
...
# Diasbled this check - not using inheritance
...
class ModelTable(Model):
    ...
    def render(self):
        met = ' Model.metadata,' #if _flask_prepend == '' else ''
...
# metadata = Base.metadata
    ...
    def render(self, outfile=sys.stdout):
        ...
        for model in self.models:
            print('\n\n#STARTCLASS', file=outfile)
            print(model.render().rstrip('\n'), file=outfile)
            print('\n#ENDCLASS', file=outfile)
        if self.footer:
